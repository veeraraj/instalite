...
extension HomeViewModel {
    ...
    func didTapMediaItem(_ mediatItem: MediaItem) {
        showMediaDetails(for: mediatItem)
    }
}
...
private extension HomeViewModel {
    ...
    func showMediaDetails(for mediaItem: MediaItem) {
        guard mediaItem.isAlbum else {
            coordinator?.showMediaDetails([mediaItem.mediaURL])
            return
        }
        
        Task {
            do {
                let albumDetails = try await mediaInfoRepository.fetchAlbumInfo(for: mediaItem.id)
                coordinator?.showMediaDetails(albumDetails.data.compactMap { $0.mediaURL })
            } catch let fetchError {
                logger.error("\(fetchError)")
                error = fetchError
            }
